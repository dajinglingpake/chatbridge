...
class BridgeMessageRuntimeTests(unittest.TestCase):
    ...
    def test_media_only_message_is_cached_for_next_text_message(self) -> None:
        ...
        self.assertEqual("看图 /tmp/a.png", submitted[0][1])
        self.assertNotIn("\n", submitted[0][1])
        self.assertNotIn("图片: a.png", submitted[0][1])
        self.assertNotIn("本地路径", submitted[0][1])
        self.assertNotIn("用户发送了以下附件", submitted[0][1])
        self.assertNotIn("必须先查看", submitted[0][1])
        self.assertNotIn("请结合用户文字", submitted[0][1])
