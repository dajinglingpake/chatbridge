...
class BridgeInterruptRuntimeTests(unittest.TestCase):
    ...
    def test_render_interrupt_prompt_stays_flat(self) -> None:
        ...
        self.assertEqual("补充一\n\n补充二", prompt)
        self.assertNotIn("原始问题", prompt)
        self.assertNotIn("<current_user_followups>", prompt)
        self.assertNotIn("<previous_user_request_context>", prompt)
    ...
    def test_render_interrupt_prompt_uses_translation(self) -> None:
        ...
        self.assertEqual("Check i18n", prompt)
        self.assertNotIn("Original request", prompt)
        self.assertNotIn("User follow-up messages", prompt)
